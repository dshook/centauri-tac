...
async function run(){
    ...
    for(let i = minSyncRow; i <= maxSyncRow; i++){
        ...
        if(foundCard){
            foundCard = replaceVal(foundCard, "name", sheetRow.name);
            foundCard = replaceVal(foundCard, "movement", +sheetRow.move);
...
//String replaces lines inside a JSON like string
function replaceVal(contents, propName, value){
