...
public class CameraFlyController : MonoBehaviour
{
	...
	static private readonly KeyCode[] keysRun = new KeyCode[] { KeyCode.LeftShift, KeyCode.LeftShift };
	...
	void HandleMouseInput(float dT)
	{
		...
		if (InputHelper.GetAxis("Mouse X") != 0f || InputHelper.GetAxis("Mouse Y") != 0f)
		{
	...
	{
		float axis1 = InputHelper.GetAxis("JoystickAxis1");
		float axis2 = InputHelper.GetAxis("JoystickAxis2");
		//float axis3 = InputHelper.GetAxis("JoystickAxis3");
		float axis4 = InputHelper.GetAxis("JoystickAxis4");
		float axis5 = InputHelper.GetAxis("JoystickAxis5");
...
static public class InputHelper
{
	...
	{
		// There is currently (Unity 5.5.0f3) no other way in Unity to check if a specific axis is defined in Input Manager or not
