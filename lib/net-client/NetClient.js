...
export default class NetClient extends EventEmitter
{
   ...
   */
  async sendCommand(name, command, params)
  {
  ...
  /**
   * GET information to a REST endpoint
   */
  async get(component, url)
  {
    // try again
    if (typeof component === 'string') {
      const c = await this.getComponent(component);
      return this.get(c, url);
    }
    ...
    this.log.info('GET %s', opts.url);

    const [response, body] = await this._transport.get(opts);
  ...
  async post(component, url, data)
  {
    ...
    this.log.info('POST %s %j', opts.url, data);
